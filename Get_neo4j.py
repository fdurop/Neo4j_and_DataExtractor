...
class Neo4jTeamCollaborator:
    ...
    def import_from_csv(self, node_file: str = None, rel_file: str = None, batch_size: int = 1000) -> None:
        ...
        try:
            ...
            nodes = None
            if node_file:
                if node_file.startswith("http"):
                    encoding = self._detect_encoding_from_url(node_file)
                    response = requests.get(node_file)
                    response.encoding = encoding
                    nodes = pd.read_csv(response.text)
                else:
                    encoding = self._detect_encoding(node_file)
                    nodes = pd.read_csv(node_file, encoding=encoding)
                print(f"读取到 {len(nodes)} 个节点")

            # 读取关系数据（如果提供了关系文件）
            rels = None
            if rel_file:
                if rel_file.startswith("http"):
                    encoding = self._detect_encoding_from_url(rel_file)
                    response = requests.get(rel_file)
                    response.encoding = encoding
                    rels = pd.read_csv(response.text)
                else:
                    encoding = self._detect_encoding(rel_file)
                    rels = pd.read_csv(rel_file, encoding=encoding)
        ...
        try:
            ...
            if node_file:
                if node_file.startswith("http"):
                    ...
                    response = requests.get(node_file)
                    response.encoding = encoding
                    nodes_data = pd.read_json(response.text)
                else:
                    encoding = self._detect_encoding(node_file)
                    with open(node_file, 'r', encoding=encoding) as f:
                        nodes_data = pd.read_json(f)
            ...
            if rel_file:
                if rel_file.startswith("http"):
                    ...
                    response.encoding = encoding
                    # 读取JSON格式的关系数据
                    rels_json = json.loads(response.text)
                    rels_data = pd.DataFrame(rels_json)
                else:
                    encoding = self._detect_encoding(rel_file)
                    with open(rel_file, 'r', encoding=encoding) as f:
                        rels_json = json.load(f)
                    rels_data = pd.DataFrame(rels_json)
